...
pdftitle={Applications of hyper-resolution principle, 
...
\title{Applications of hyper-resolution principle}
...
In 1965, Robinson presented a first order logic which is specifically designed for use as theoretical instrument of a computer theorem-proving program \cite{robinson}. A prominent feature of this logic system is that is based on one single inference principle, the resolution principle. 
...
The hyper-resolution principle is a generalization \cite{rob,robinson-general} that simplifies the steps 
\section{Formal prelimaries}

The syntax of the hyper-resolution calculs 
...
\end{definition}
The \textit{resolution principle} can be stated as follow:
...
\end{tabular}
\end{center}
This two trees are resolution trees, both with only one application of the resolution principle. If the clauses are interpreted as disjunction of their elements, the second tree has an analogous made with the rules of natural deduction 
\begin{equation*}
...
\noindent\textbf{Example} Let $T=\{ A \e B \rightarrow C \o D ,\; A,\; B,\; \neg C \}$, then $H_T$ has four rules:
...
Now Robinson's theorem can be state as follows
\begin{theorem}[Robinson's theorem, version 1]
If $T$ is contradictory, then $\vdash_{H_T} \emptyset$
\end{theorem}
 

Lifschitz \cite{lifschitz} this theorem in order to be applied to 
...
\begin{theorem}[\textbf{Robinson's , version 3}] \label{Robinson3}
...
Given a rule in the form (\ref{axiom}) and an application of it in a derivation, whenever $A_i \in \Gamma_i$ for some $i$, we call such application \textit{trivial}. The following two lemmas ensure that such kind of applications can always be removed.
...
\textit{Proof:} Consider a trivial application of a rule in a given derivation

$$
\prftree{\prfsummary[$D_1$]{}{\Sigma_1}}{\cdots}{\prfsummary[$D_m$]{}{\Sigma_m}}
{\prfsummary{\Pi}{}}
$$
Say, without loss of generality, that $\Sigma_1\subseteq\Pi$\\ ....

This last lemma and Theorem \ref{Robinson3}  let us give a final version of Robinson's theorem.
...
\end{proposition}
\textit{Proof:} 
Let $G=(V,E)$; we proceed by induction on Definition \ref{def_mun}. If $G$ is a complete graph with $n+1$ vertices, clearly is not $n$-colourable. Then let $G_1$ and $G_2$ as in the Definition \ref{def_mun}; by inductive hypothesis they are not  $n$-colourable, then the empty set is derivable in both calculus $H_{G_1,n}$ and $H_{G_2,n}$.
...
\noindent Indeed, by Kuratowski's Theorem any graph that has $K^5$ as minor, is not planar. For $n=1$, the conjecture is trivially true. For $n=2$, it follows from Proposition \ref{contains-odd}, since any cycle has $K^3$ as minor. 
