...
Formally an interpretation is a mapping $I$ of a \textit{language} $\mathscr{L}$ onto a collection of entities. Specifically $I$ maps every function symbol of degree $n$ in $\mathscr{L}$ onto a function from $D^n$ to $D$, and each relation symbol of degree $n$ onto a function from $D^n$ to $\{\mathbf{true, false}\}$. 
...
We say that, if the empty set is the conclusion of some resolution tree, such tree is a \textit{refutation} of its leaves; the soundness theorem ensures that if there is a refutation for a set of clauses $S$, such set is also \textit{unsatisfiable}. Robinson's theorem \cite{robinson} give us also the completeness.
